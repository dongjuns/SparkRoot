...
out_f = open("/home/sunyoung/nano/NanoAOD/TTHAnalyzer/out.txt","w")

#print("number of data : ", col)
out_f.write(str(col))

#sample_num = sql("""select HLT_AK4CaloJet100
#		from NanoAOD""").count()
...
MuonSelectionQuery = """ select event e, 
			 from NanoAOD
			 where 20 < all (select max(x) from unset(e.Muon_Pt) as x)
			"""
			# where MAX(Muon_Pt) > 20
			#"""
			#	Muon_Pt[0] > 20 
			#	and Muon_Eta[0] < 2.4 
			#	and Muon_Eta[0] > -2.4 
			#	and Muon_pfRelIso04_all[0] < 0.25 
			#	"""


sample_num = sql(MuonSelectionQuery).count()
print("muon num : ", sample_num)
...
### Open Root File ###
filelist = ["/home/nanoAOD/GluGlu_HToMuMu_M125_13TeV_powheg_pythia8/run2_2016MC_NANO_RunIISummer16MiniAODv2-PUMoriond17_80X_mcRun2_asymptotic_2016_TrancheIV_v6-v1/171112_002433/0000/run2_2016RD_NANO_1.root"
            ]
NanoFiles = ["/home/sunyoung/nano/NanoAOD/TTHAnalyzer/run2_2016MC_NANO_2.root"]      
...
print("Time duration : ", etime-stime)
